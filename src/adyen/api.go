...
import (
	"fmt"
	"github.com/bos-hieu/adyen-go-api-library/src/referrals"
	"net/http"
...
const (
	...
	ReferralsEndpointTest      = "https://ca-test.adyen.com/ca/services/ReferralCAService"
	ReferralsEndpointLive      = "https://ca-live.adyen.com/ca/services/ReferralCAService"
)
...
func NewClient(cfg *common.Config) *APIClient {
	...
	c.Referrals = &referrals.Referrals{
		Client: c.client,
		BasePath: func() string {
			return fmt.Sprintf("%s", c.client.Cfg.ReferralsEndpoint)
		},
	}
...
func (c *APIClient) SetEnvironment(env common.Environment, liveEndpointURLPrefix string) {
	if env == common.LiveEnv {
		...
		c.client.Cfg.ReferralsEndpoint = ReferralsEndpointLive
		...
		c.client.Cfg.ReferralsEndpoint = ReferralsEndpointTest
	}
}
